...
void print_help(int exval) {
	fprintf(stderr,"%s,%s by CW\r\n", PACKAGE, VERSION_SW);
	fprintf(stderr,"%s [-h] [-V] [-a My_Addr] [-r RxDemo_Addr] [-i Msg_Interval] [-t tx_retries] [-c channel] [-f frequency]\r\n", PACKAGE);
	fprintf(stderr,"          [-m modulation]\n\r\n\r");
	fprintf(stderr,"  -h              			print this help and exit\r\n");
	fprintf(stderr,"  -V              			print version and exit\r\n\r\n");
	fprintf(stderr,"  -v              			set verbose flag\r\n");
	fprintf(stderr,"  -a my address [1-255] 		set my address\r\n\r\n");
	fprintf(stderr,"  -r rx address [1-255] 	  	set RxDemo receiver address\r\n\r\n");
	fprintf(stderr,"  -i interval ms[1-6000] 	  	sets message interval timing\r\n\r\n");
	fprintf(stderr,"  -t tx_retries [0-255] 	  	sets message send retries\r\n\r\n");
	fprintf(stderr,"  -c channel 	[1-255] 		set transmit channel\r\n");
	fprintf(stderr,"  -f frequency  [315,434,868,915]  	set ISM band\r\n\r\n");
	fprintf(stderr,"  -m modulation [100,250,500] 		set modulation\r\n\r\n");
...
int main(int argc, char *argv[]) {
	...
	while((opt = getopt(argc, argv, "hVva:r:i:t:c:f:m:")) != -1) {
		switch(opt) {
			...
			case 'V':
				...
				break;
			case 'v':
				fprintf(stderr,"%s: Verbose option is set `%c'\n", PACKAGE, optopt);
				cc1100_debug = 1;
				break;
			case 'a':
				My_addr = atoi (optarg);
	...
	//------------- welcome message ------------------------
	fprintf(stderr,"Raspberry CC1101 SPI Library test\n");
...
*/
			...
			{
				fprintf(stderr,"transmitted tx_timestamp: %ums \r\n\r\n", time_stamp);
			}
